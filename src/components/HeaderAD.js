...
export default function Header() {
  ...
  const handleSearch = () => {
    ...
    } else if (searchType === 'name') { // Assuming 'name' as a new search type for searching by movie name
      navigate(`/movie?search=${searchQuery}`);
    }
  };
      ...
      <Container>
              ...
              {userId === -1 ? (
                ...
                <Nav className="flex-child-menu navbar-right">
                  <Nav className="flex-child-menu menu-right" style={{ display: 'flex', marginTop: '16px' }}>
                    ...
                    <Nav.Link href="/admin/movie-list" style={{ margin: '6px' }}><i className="fa fa-user" style={IconStyle}></i></Nav.Link>
                  </Nav>
                </Nav>
              )}
        ...
        <div className="top-search" style={{ border: 'none', width: '80%', margin: 'auto' }}>
          <FormControl as="select" onChange={(e) => setSearchType(e.target.value)}>
            ...
            <option value="name">Name</option> {/* Add 'Name' as a search option */}
          </FormControl>
          <FormControl
            type="text"
